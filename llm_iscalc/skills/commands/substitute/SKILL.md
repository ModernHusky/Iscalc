...
### 正向换元 `substitute u for <expr>`

1. **设定换元**：令 `u = <expr>`
2. **求导**：计算 `du = d(<expr>)/dx * dx`
3. **替换被积函数**：将 `<expr>` 替换为 `u`，`dx` 替换为 `du / (d<expr>/dx)`
4. **变换边界**（定积分）：计算新的积分上下界

### 反向换元 `substitute <expr> for x`

1. **设定换元**：令 `x = <expr>`（其中 `<expr>` 含新变量 `u`）
2. **求导**：计算 `dx = d(<expr>)/du * du`
3. **替换被积函数**：将 `x` 替换为 `<expr>`
4. **变换边界**（定积分）：求解新的积分上下界
...
4. **变量恢复（仅限不定积分）**：
   - 在计算完成后，**必须**使用 `replace substitution` 将中间变量 `u` 换回原变量 `x`
   - **定积分**不需要此步骤，因为积分限已经变换

5. **常见错误**：
   - `incorrect old variable y, should be x`：试图替换非积分变量
   - 确保新变量名在之前的步骤中未被定义或占用
...
> 💡 `<|load_skill|>rewrite<|end_load_skill|>` — 换元后进行代数化简

> 💡 `<|load_skill|>apply-integral-identity<|end_load_skill|>` — 应用积分恒等式
